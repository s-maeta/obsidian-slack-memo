...
**📝 注意：** 
- トークンは`xoxb-`で始まる必要があります。`xoxp-`や`xoxe-`で始まるトークンは使用できません。
- 設定情報は`data.json`に保存されます。このファイルにはトークンが含まれるため、他人と共有しないでください。

### 4. セキュリティ設定ファイル

プラグインの設定は`data.json`ファイルに保存されます。このファイルには機密情報（Slackトークン）が含まれるため：

- ✅ **Git管理から除外済み** - `.gitignore`で自動除外
- ✅ **サンプルファイル提供** - `data.json.example`を参考にセットアップ可能
- ⚠️ **バックアップ時は注意** - 設定をバックアップする際はトークンを削除してください
...
### 基本設定
- **Slackトークン** - Bot User OAuth Token (xoxb-で始まる)
- **保存先フォルダ** - メッセージの保存場所（デフォルト: "Slack Sync"）
- **同期対象チャンネル** - 同期するチャンネルを選択
- **自動同期** - 自動同期のオン/オフ
- **同期間隔** - 自動同期の実行間隔（1〜60分）
...
### Q: 過去のメッセージも同期されますか？
A: 初回同期時は過去24時間分のメッセージを取得します。その後は最終同期時刻以降の新しいメッセージのみ同期されます。

### Q: 同期履歴はどこに保存されますか？
A: チャンネルごとの最終同期時刻や統計情報は `data.json` に自動保存されます。重複メッセージの取得を防ぎ、効率的な同期を実現します。
